...
class Operator(util.OperatorBase):
    def __init__(self, device_id, data_path, device_name='das Gerät'):
        ...
        self.window_boundaries_times =  [datetime.time(0, 0), datetime.time(1, 0), datetime.time(2, 0), datetime.time(3, 0), datetime.time(4, 0), datetime.time(5, 0), datetime.time(6, 0),
                                         ...
                                         datetime.time(21, 0), datetime.time(22, 0), datetime.time(23, 0), datetime.time(23, 59,999000)]
            ...
            if self.current_five_min==self.todatetime(self.consumption_same_five_min[-1]['Time']).tz_localize(None).floor('5T'):
                ...
                if len(self.time_window_consumption_list_dict[str(self.last_time_window_start)]) >= 24:
                    ...
                    if self.timestamp in list(chain.from_iterable(days_with_excessive_five_min_consumption_during_this_time_window_of_day)):
                        return {'value': f'Nachricht vom {str(self.timestamp.date())} um {str(self.timestamp.hour)}:{str(self.timestamp.minute)} Uhr: Im letzten Zeitfenster wurde durch {self.device_name} übermäßig viel verbraucht.'} # Excessive time window consumption just detected.
                    else:
